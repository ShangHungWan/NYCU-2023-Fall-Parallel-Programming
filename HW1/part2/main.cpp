...
void initValue(float *values1, float *values2, double *values3, float *output, unsigned int N)
{
  ...
  {
    // random input values
    values1[i] = -1.f + 4.f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
    values2[i] = -1.f + 4.f * static_cast<float>(rand()) / static_cast<float>(RAND_MAX);
    values3[i] = -1.f + 4.f * static_cast<double>(rand()) / static_cast<float>(RAND_MAX);
    output[i] = 0.f;
  }
}
