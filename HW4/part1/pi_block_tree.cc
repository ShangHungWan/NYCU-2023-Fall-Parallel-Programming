...
int main(int argc, char **argv)
{
    ...
    for (int i = 2; i <= world_size; i *= 2)
    {
        if (world_rank % i != 0)
        {
            MPI_Send(&private_number_in_circle, 1, MPI_LONG_LONG_INT, world_rank - i / 2, 0, MPI_COMM_WORLD);
            break;
        }
        if (world_rank + i / 2 < world_size)
        {
            long long int temp;
            MPI_Recv(&temp, 1, MPI_LONG_LONG_INT, world_rank + i / 2, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            private_number_in_circle += temp;
        }
    }

    if (world_rank == 0)
    {
        pi_result = 4 * private_number_in_circle / ((double)tosses);
