...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        long long int *number_in_circle = new long long int[world_size - 1];
        for (int i = 1; i < world_size; i++)
        {
            MPI_Irecv(&number_in_circle[i - 1], 1, MPI_LONG_LONG_INT, i, 0, MPI_COMM_WORLD, &requests[i - 1]);
        }
        ...
        for (int i = 1; i < world_size; i++)
        {
            private_number_in_circle += number_in_circle[i - 1];
        }
